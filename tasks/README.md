# Tasks — Desktop Mode via PhotinoAOT

This folder contains the elementary, testable tasks required to add a third
usage type to CSAgent: a **desktop** mode based on
[PhotinoAOT](https://github.com/lixinyang123/PhotinoAOT).

## Goal

Add a `--desktop` flag that opens a native Photino window. The desktop mode:
...
- Each task is **simple** and **testable**.
- The project **must compile** at the end of every task.
- If a task feels too heavy or complex, **split it into sub-tasks**.
...
See `PROGRESS.md` for the current step.
